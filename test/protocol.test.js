...
test("protocol serializes minimal display snapshots for cross-world transport", () => {
  const text = protocol.serialize([
    { id: "secret-id", name: " Home ", icon: "STAR", lat: "1.5", long: "2.5", extra: "nope" },
    { name: "Bad", lat: 99, long: 1 },
    null
  ]);
  assert.equal(typeof text, "string");
  const locations = protocol.message(text);
  assert.deepEqual(locations, [{ name: "Home", icon: "star", lat: 1.5, long: 2.5 }]);
...
test("protocol rejects malformed, oversized, and out-of-schema text transport", () => {
  const validLocation = { name: "A", icon: "pin", lat: 1, long: 2 };
  ...
  assert.equal(protocol.message(JSON.stringify({ version: 1, locations: [{ ...validLocation, id: "private" }] })), null);
  assert.equal(protocol.message(JSON.stringify({ version: 1, locations: [{ ...validLocation, lat: "1" }] })), null);
  assert.equal(protocol.message(JSON.stringify({ version: 1, locations: [{ ...validLocation, icon: "unknown" }] })), null);
  assert.equal(protocol.message(JSON.stringify({ version: 1, locations: [{ ...validLocation, name: "x".repeat(121) }] })), null);
  assert.equal(protocol.message(JSON.stringify({ version: 1, locations: Array.from({ length: protocol.MAX_LOCATIONS + 1 }, () => validLocation) })), null);
  assert.equal(protocol.message({ version: 1, locations: [validLocation] }), null);
});

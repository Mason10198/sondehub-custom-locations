...
(function exposeLocations(root, factory) {
  ...
  function validateLocation(input, options) {
    ...
    return { ok: true, value: Object.freeze({ id: requestedId || (allowGeneratedId ? createId() : ""), name, icon: normalizeIcon(input && input.icon), lat: latitude, long: longitude }) };
  }
  ...
  function importCsv(text) {
    ...
    const indices = Object.fromEntries(required.map((column) => [column, header.indexOf(column)]));
    ...
    for (let physicalIndex = headerIndex + 1; physicalIndex < rows.length; physicalIndex += 1) {
      ...
      const result = validateLocation({ name: row[indices.name], icon: row[indices.icon], lat: row[indices.lat], long: row[indices.long] });
  ...
  return { DEFAULT_ICON, ICONS, normalizeIcon, createId, validateLocation, parseCsv, importCsv };
});

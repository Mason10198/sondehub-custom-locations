...
(function exposeProtocol(root, factory) {
  ...
  const MAX_TRANSPORT_CHARS = 102400;
  ...
  function displayLocation(input) {
    try {
      ...
      return Object.freeze({ name: value.name, icon: value.icon, iconColor: value.iconColor, backgroundColor: value.backgroundColor, markerDiameter: value.markerDiameter, lat: value.lat, long: value.long });
    } catch (_) {
      return null;
    }
  }
  ...
  function parsedLocation(value) {
    if (!value || typeof value !== "object" || Array.isArray(value) || !hasExactKeys(value, ["name", "icon", "iconColor", "backgroundColor", "markerDiameter", "lat", "long"])) return null;
    if (typeof value.name !== "string" || typeof value.icon !== "string" || typeof value.iconColor !== "string" || typeof value.backgroundColor !== "string" || typeof value.markerDiameter !== "number" || typeof value.lat !== "number" || typeof value.long !== "number") return null;
    ...
    if (locationsApi.markerDiameterFrom(value.markerDiameter, null) !== value.markerDiameter) return null;
